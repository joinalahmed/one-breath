...
export const PhotoLibraryScreen: React.FC<PhotoLibraryScreenProps> = ({ photoLibrary }) => {
  ...
  return (
    <div className="space-y-3 px-3">
      {/* Header */}
      <div className="mb-3">
        <h2 className="text-lg font-black text-amber-300 uppercase tracking-wide">Species Compendium</h2>
        <p className="text-[11px] text-slate-400">Your underwater discoveries • {discoveredItems.length} species catalogued</p>
      </div>
                  ...
                  >
                    <motion.div
                      ...
                      className={`relative w-full aspect-[4/5] rounded-2xl overflow-hidden border-2 transition-all duration-300 ${
                        isSelected ? `border-cyan-400 ${info.color.border}` : 'border-slate-700/50 hover:border-slate-600'
                      }`}
                      style={{
      ...
      {/* Stats Footer */}
      <div className="grid grid-cols-4 gap-2 text-center text-xs pt-3 pb-2">
        <motion.div whileHover={{ scale: 1.05 }} className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
          <p className="text-slate-400 font-bold text-[9px]">TOTAL</p>
          <p className="text-base font-black text-slate-300">{discoveredItems.length}</p>
        </motion.div>
        <motion.div whileHover={{ scale: 1.05 }} className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
          <p className="text-slate-400 font-bold text-[9px]">COMMON</p>
          <p className="text-base font-black text-slate-300">
            {discoveredItems.filter(([type]) => ['oyster', 'fish'].includes(type)).length}
          </p>
        </motion.div>
        <motion.div whileHover={{ scale: 1.05 }} className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
          <p className="text-slate-400 font-bold text-[9px]">RARE</p>
          <p className="text-base font-black text-blue-300">
            {discoveredItems.filter(([type]) => ['seahorse', 'crab', 'eel'].includes(type)).length}
          </p>
        </motion.div>
        <motion.div whileHover={{ scale: 1.05 }} className="p-1.5 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 transition-colors">
          <p className="text-slate-400 font-bold text-[9px]">EPIC+</p>
