...
export const StreakComboBanner: React.FC<StreakComboBannerProps> = ({ streak, onDismiss }) => {
  ...
  return (
    <motion.div
      ...
      onClick={onDismiss}
      className="relative z-30 w-full mb-3 p-0.5 rounded-2xl bg-gradient-to-r from-amber-400/40 via-rose-500/40 to-amber-400/40 shadow-[0_0_25px_rgba(251,191,36,0.2)] overflow-hidden cursor-pointer"
    >
...
// -------------------------------------------------------------
// 3. DEFEAT / LOSE OVERLAY MODAL (Shark Attack / Out of Air)
// -------------------------------------------------------------
interface DefeatModalProps {
  outcome: 'shark' | 'drowned';
  maxDepth: number;
...
export const DefeatModal: React.FC<DefeatModalProps> = ({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 select-none overflow-hidden"
    >
      ...
      <motion.div
        ...
        className="relative z-10 max-w-sm w-full bg-slate-900 border-2 border-rose-500/80 rounded-3xl p-5 text-center shadow-[0_0_50px_rgba(244,63,94,0.4)] flex flex-col items-center"
      >
        {/* Dark Red Vignette Flash */}
        <div className="absolute inset-0 bg-rose-950/30 rounded-3xl pointer-events-none animate-pulse" />

        {/* Defeat Badge */}
        <motion.div
          animate={{ scale: [0.8, 1.15, 1], rotate: [0, -8, 8, 0] }}
          transition={{ duration: 0.4 }}
          className={`w-20 h-20 rounded-3xl p-1 flex items-center justify-center my-2 shadow-2xl ${
            isShark
        ...
        <h2 className="text-2xl font-black text-rose-400 tracking-tight uppercase">
          {isShark ? 'SHARK ATTACK!' : 'OUT OF AIR!'}
        </h2>
        <p className="text-xs font-bold text-slate-300 mt-1 max-w-[260px]">
          {isShark
            ? 'A reef shark intercepted your ascent! Basket items were dropped.'
            : 'You ran out of air before reaching the water surface! Basket spilled.'}
        </p>

        {/* Loss Breakdown Box */}
        <div className="my-4 w-full bg-slate-950/80 border border-slate-800 rounded-2xl p-3 text-left space-y-2 text-xs">
          <div className="flex justify-between items-center border-b border-slate-800 pb-1.5">
            <span className="text-slate-400 font-semibold">Max Depth Reached:</span>
            <span className="font-mono font-bold text-cyan-300">{maxDepth}m</span>
          </div>

          <div className="flex justify-between items-center border-b border-slate-800 pb-1.5">
            <span className="text-slate-400 font-semibold">Streak Outcome:</span>
            <div className="flex items-center space-x-1 font-mono">
              <span className="text-slate-400 line-through font-bold">{previousStreak}x</span>
              <span className="text-rose-400 font-black">➔ 1x (Reset)</span>
            </div>
          </div>

          <div className="flex justify-between items-center">
            <span className="text-slate-400 font-semibold">Safety Net Salvage:</span>
            <span className="font-mono font-bold text-amber-300">+0 💎 (Basket Spilled)</span>
          </div>
        </div>
        ...
        <div className="w-full space-y-2">
          <motion.button
            ...
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-600 text-white font-black text-sm tracking-wider uppercase shadow-xl cursor-pointer flex items-center justify-center space-x-2"
          >
            <span>🤿</span>
            <span>RETRY DIVE IMMEDIATELY!</span>
          </motion.button>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-2xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs uppercase tracking-wider transition-colors cursor-pointer"
          >
            RETURN TO VILLAGE
          </button>
        </div>
